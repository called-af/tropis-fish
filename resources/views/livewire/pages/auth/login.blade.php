    ...
    <div class="w-full lg:w-1/2 xl:w-2/5 bg-gray-900 flex items-center justify-center p-6 sm:p-8 lg:p-12">
        <div class="w-full max-w-md">
            ...
            <form wire:submit="login" class="space-y-6">
                ...
                @if (session('error'))
                    <div
                        class="bg-red-500/20 border border-red-500/50 text-red-200 px-4 py-3 rounded-2xl font-light tracking-wide text-sm">
                        {{ session('error') }}
                    </div>
                @endif
                ...
                {{-- Email Input --}}
                <div>
                    <x-atoms.input type="email" label="Email Address" wire:model="email"
                        placeholder="admin@tropisfish.com" required :error="$errors->first('email')" />
                </div>

                {{-- Password Input --}}
                <div>
                    <x-atoms.input type="password" label="Password" wire:model="password"
                        placeholder="Enter your password" required :error="$errors->first('password')" />
                </div>

                {{-- Submit Button --}}
                <div class="pt-2">
                    <x-atoms.button type="submit" variant="primary" size="lg" icon="arrow-right-on-rectangle"
                        class="w-full shadow-xl shadow-blue-900/50" wire:loading.attr="disabled">
                ...
                <div class="text-center pt-4">
                    <a href="{{ route('home') }}"
                        class="text-sm text-amber-500 hover:text-amber-500 font-light tracking-wide transition-colors duration-300 inline-flex items-center gap-2">
